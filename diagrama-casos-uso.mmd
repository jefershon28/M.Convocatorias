%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#2FC6F6', 'primaryTextColor': '#fff', 'primaryBorderColor': '#1a9bc4', 'lineColor': '#333', 'secondaryColor': '#e8f7fc', 'tertiaryColor': '#f5f5f5'}}}%%
flowchart TB
    subgraph Sistema["SISTEMA: Módulo Condiciones, Anticipos y Descuentos"]
    ...
    RRHH(("👤 RRHH<br/>Personal Administrativo"))

    %% Asociaciones principales del actor
    RRHH --> UC01
    RRHH --> UC04
    RRHH --> UC06
    RRHH --> UC09
    ...
    %% INCLUDE: caso base incluye al otro (flujo obligatorio)
    ...
    %% EXTEND: caso que extiende al base (flujo opcional bajo condición)
    %% Dirección: caso que extiende -.->|extend| caso base
    UC26 -.->|extend| UC25
    UC27 -.->|extend| UC25
    UC28 -.->|extend| UC25
    UC29 -.->|extend| UC25
